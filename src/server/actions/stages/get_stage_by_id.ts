...
export const getStageById = defineAction({
    ...
    handler: async (
        ...
        let commentsData: PartialCommentPreview[] | null | undefined =
            undefined;

        if (data.allowComments) {
            if (!fields || fields.some((f) => f.includes("comments"))) {
                const data = await db
                    ...
                    .innerJoin(
                        StagesTable,
                        and(
                            eq(CommentsTable.stageId, StagesTable.id),
                            eq(CommentsTable.tripId, StagesTable.tripId),
                            eq(StagesTable.allowComments, true)
                        )
                    )
                    .where(
                        and(
        ...
        return filterObject(
            {
                ...
                comments: data.allowComments ? commentsData : null,
